// Lab 10b, ElevatorSimulation3, Elevator.cpp
...
void Elevator::setDestinationBasedOnRiders()
{
  ...
  {
    //calculate distance
    calcDistance = getLocation() - riders[i].getDestination().getLocation();

    //convert to absolute value
    calcDistance = abs(calcDistance);
    if(getLocation() - riders[i].getDestination().getLocation() < 0 ||
       getLocation() - riders[i].getDestination().getLocation() > calcDistance)
         calcDistance = getLocation() - riders[i].getDestination().getLocation();
